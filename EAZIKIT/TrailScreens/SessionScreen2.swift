...
class SessionScreen2: UIViewController,UIImagePickerControllerDelegate, UINavigationControllerDelegate, UNUserNotificationCenterDelegate {
    ...
    override func viewDidLoad() {
        ...
        sessionTime = "\(PRE_SESSION_TIME.prefix(2))"
        ...
        if let decodedImageData = Data(base64Encoded: PRE_SESSION_IMAGE, options: .ignoreUnknownCharacters) {
             self.beforeTeethImage.image = UIImage(data: decodedImageData)
        }
...
//        teethimageView.stopAnimating()
    ...
    func saveWithImage()
    {
        ...
        for aViewController in viewControllers {
            if aViewController is FivthIntoScreen {
